...
async function formatDiscordText(
	text: string,
	colors: ProfileColors,
	username: string
) {
	...
	formattedText = formattedText
		.replace(
			/\*\*(.*?)\*\*/g,
			`<strong style="color: ${getElementColor(
				colors,
				"bold_text"
			)}">$1</strong>`
		)
		.replace(/\[(.*?)\]\((.*?)\)/g, (match, text, url) => {
			...
			return `<a href="javascript:void(0)" data-url="${url}" class="${
				colors.elements.text_underline.type === "linear"
					? "text-blue-400 hover:underline"
					: ""
			}" style="${
				colors.elements.text_underline.type === "gradient"
					? `border-bottom: 2px solid ${getElementColor(
							colors,
							"text_underline"
					  )}`
					: ""
			}" onclick="window.handleExternalLink(this)">${cleanText}</a>`;
		})
		.replace(
			urlPattern,
			(url) =>
...
export default function ProfilePage({
	...
	useEffect(() => {
		async function formatBio() {
			if (profile?.bio) {
				...
				try {
					const formatted = await formatDiscordText(
						profile.bio,
						profile.colors,
						params.username
					);
	...
	const spotify = profile.presence?.activities?.find(
			...
			<div className="w-full max-w-[700px]">
				...
				>
					<div className="relative">
						...
						<div
							className={`${
								profile.glass_effect ? "bg-black/50" : "bg-black"
							} p-6`}
						>
									...
									<div className="relative">
										...
										<div
											className={`absolute bottom-2 right-2 h-4 w-4 rounded-full ${
												statusColors[
													(profile.presence
														?.status as keyof typeof statusColors) || "offline"
												]
											} ${
												profile.colors.elements.status.type === "linear" &&
												(profile.colors.elements.status as LinearColor).color
													? "ring-2"
													: ""
											}`}
											style={{
									...
									<div className="text-white/80 flex items-center gap-1">
										{isBioLoading ? (
											...
											<div
												...
												style={{
													...(profile.colors.elements.bio.type === "gradient"
														? {
														...
														: {
																color: getElementColor(profile.colors, "bio"),
														  }),
												}}
												...
												{activity && activity.album_cover_url ? (
													...
													activity.large_image && (
														<Image
															...
															alt={activity.large_text || "Activity image"}
															unoptimized
															width={60}
															height={60}
															className="rounded-md"
												...
												<div>
													...
													{profile.presence.activities.length > 1 && (
														<div className="mt-2 flex items-center gap-2">
															{profile.presence.activities.map((_, index) => (
																<button
																	key={index}
												...
												<div className="flex-1 min-w-0 text-center sm:text-left">
													...
													<div className="flex gap-4 mt-2 text-sm text-white/60 justify-center sm:justify-start flex-wrap">
														...
														<div className="flex items-center gap-1">
															...
															{guildData.member_count.toLocaleString()} members
														</div>
													</div>
												</div>
												<a
				...
				<DialogContent className="bg-[#111111] border-[#222222] text-white p-6 rounded-xl max-w-md">
					<div className="space-y-4">
						...
						<p className="text-[#989898]">
							You are leaving this site to visit an external website. Are you
							sure you want to continue?
							<br />
							<br />
							We are not responsible for any issues that may arise from visiting
							this website.
						</p>

						<div className="bg-[#0a0a0a] p-3 rounded-lg border border-[#222222] break-all">
							<span className="text-[#989898] font-medium">
								{linkModal.url}
							</span>
						</div>
			...
			<Dialog open={reportModal && !!session} onOpenChange={setReportModal}>
				<DialogContent className="bg-[#111111] border-[#222222] text-white p-6 rounded-xl max-w-md">
					<div className="space-y-4">
						...
						<p className="text-[#989898]">
							Please select a reason and provide additional details for
						...
						<select
							...
							onChange={(e) =>
								setSelectedReason(
									e.target.value as (typeof reportReasons)[number]
								)
							}
						...
						<div className="flex justify-end gap-3 pt-2">
							...
							<button
								...
								disabled={
									!selectedReason || !reportReason.trim() || isSubmitting
								}
