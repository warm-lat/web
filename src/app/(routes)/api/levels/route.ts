...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const response = await fetch(
			`https://api.warm.lat/levels?_=${Date.now()}`,
			{
				headers: {
		...
		if (!response.ok) {
			throw new Error(
				`API responded with status: ${response.status}. If you are a user, please contact @_mism. on Discord.`
			);
		}
		...
		return NextResponse.json(
			{
				error:
					"Internal Server Error. Guild most likely does not have any level data or does not exist.",
			},
			{ status: 500 }
		);
	}
