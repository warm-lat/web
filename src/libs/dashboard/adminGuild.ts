...
export async function fetchGuildRoles(guildId: string): Promise<RolesResponse> {
	return fetchWithCache(`roles-${guildId}`, async () => {
		...
		const response = await fetch(`https://api.warm.lat/roles`, {
			headers: {
				Authorization: `Bearer ${token}`,
				"Content-Type": "application/json",
				"X-GUILD-ID": guildId,
...
export async function fetchGuildChannels(
	...
	return fetchWithCache(`channels-${guildId}`, async () => {
		...
		const response = await fetch(`https://api.warm.lat/channels`, {
			headers: {
				Authorization: `Bearer ${token}`,
				"Content-Type": "application/json",
				"X-GUILD-ID": guildId,
...
export async function fetchGuildSettings(
	...
	const response = await fetch(
		`https://api.warm.lat/guild-settings?guild_id=${guildId}`,
		{
			headers: {
