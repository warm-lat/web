...
export async function fetchGuildGreetings(
	guildId: string
	...
	const response = await fetch(`https://api.warm.lat/greetings`, {
		headers: {
...
export async function updateGuildGreetings(
	guildId: string,
	data: GreetingsUpdate
): Promise<void> {
	const token = localStorage.getItem("userToken");
	if (!token) {
		throw new Error("Unauthorized");
